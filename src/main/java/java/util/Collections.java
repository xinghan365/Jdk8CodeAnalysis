 ...
 *
 * 该类中包含的多态算法的文档通常包括实现的简要说明 。
 * 这些描述应被视为实施说明 ，而不是说明书的一部分 。 只要规范本身得到遵守，实现者就可以随意替代其他算法。
 * （例如，sort使用的算法不一定是一个mergeesort，但它必须是稳定的 。）
 *
...
public class Collections {
    ...
    /**
     * 根据其元素的natural ordering ，按照升序排列指定的列表。 列表中的所有元素必须实现Comparable接口。
     * 此外，列表中的所有元素都必须相互可比较 （即e1.compareTo(e2)不能为ClassCastException中的任何元素e1和e2 ）。
     * 这种保证是稳定的 ：等同的元素将不会被排序作为排序的结果。
     *
     * 指定的列表必须是可修改的，但不能调整大小。
     *
     * 实现注意事项：这个实现是一个稳定的，自适应的，迭代的mergeesort，
     * 当输入数组被部分排序时，需要比n lg（n）的比较少得多，同时在输入数组被随机排列时提供传统的mergeesort的性能。
     * 如果输入数组几乎被排序，则实现需要大致n个比较。 临时存储要求从几乎排序的输入数组的小常数到随机排序的输入数组的n / 2对象引用。
     *
     * 该实现在其输入数组中具有上升和下降的同等优势，并且可以在同一输入数组的不同部分中利用升序和降序。
     * 它非常适合合并两个或多个排序的数组：简单地连接数组并排序生成的数组。
     *
     * 实施从Tim Peters的Python列表排序（ TimSort ）进行了改编 。
     * 它使用Peter McIlroy的“乐观排序和信息理论复杂性”中的技术，在第四届ACM-SIAM离散算法研讨会论文集，
     * pp 467-474，1993年1月。
     *
     * 该实现将指定的列表转储到数组中，对数组进行排序，并在列表中迭代，从列表中的相应位置重新设置每个元素。
     * 这样可以避免尝试将链表排序到目前为止的n 2 log（n）性能。
     *
     ...
     * （即， c.compare(e1, e2)不能为ClassCastException中的任何元素e1和e2 ）。
     * 这种保证是稳定的 ：等同的元素将不会被排序作为排序的结果。
     *
     * 指定的列表必须是可修改的，但不能调整大小。
     *
     * 实现注意事项：这个实现是一个稳定的，自适应的，迭代的mergeesort，当输入数组被部分排序时，
     * 需要比n lg（n）的比较少得多，同时在输入数组被随机排列时提供传统的mergeesort的性能。
     * 如果输入数组几乎被排序，则实现需要大致n个比较。 临时存储要求从几乎排序的输入数组的小常数到随机排序的输入数组的n / 2对象引用。
     *
     * 该实现在其输入数组中具有上升和下降的同等优势，并且可以在同一输入数组的不同部分中利用升序和降序。
     * 它非常适合合并两个或多个排序的数组：简单地连接数组并排序生成的数组。
     *
     * 该实现从Tim Peters的Python列表（ TimSort ）进行了改编 。 它使用Peter McIlroy的“乐观排序和信息理论复杂性”
     * 中的技术，在第四届ACM-SIAM离散算法研讨会论文集，pp 467-474，1993年1月。
     *
     * 该实现将指定的列表转储到数组中，对数组进行排序，并在列表中迭代，从列表中的相应位置重新设置每个元素。
     * 这样可以避免尝试将链表排序到目前为止的n 2 log（n）性能。
     *
    ...
    /**
     * 使用二叉搜索算法搜索指定对象的指定列表。 该列表必须根据被按升序排列natural ordering元素
     * （如由sort(List)方法）之前使该呼叫。 如果没有排序，结果是未定义的。
     * 如果列表包含与指定对象相等的多个元素，则不能保证将找到哪个元素。
     * 该方法以log（n）时间运行“随机访问”列表（提供近常数位置访问）。
     * 如果指定的列表没有实现RandomAccess接口并且很大，则该方法将执行基于迭代器的二进制搜索，
     * 执行O（n）链接遍历和O（log n）元素比较。
     *
     ...
     *         搜索关键字的索引，如果它包含在列表中; 否则， (-(insertion point) - 1) 。
     *         如果列表中的所有元素都小于指定键的第一元件比所述键时，或list.size()的索引：
     *         插入点被定义为将键插入到列表中的点。 请注意，这确保当且仅当找到该键时返回值将为> = 0。
     * @throws ClassCastException if the list contains elements that are not
     ...
     * 在前面的描述中使用了“大约”对冲，因为默认的随机源仅仅是独立选择的比特的无偏差源。
     * 如果它是随机选择位的完美来源，则算法将选择具有完美均匀性的排列。
     *
     * 该实现从最后一个元素向后移动列表，重复地将随机选择的元素交换到“当前位置”中。
     * 从从第一个元素运行到当前位置（包括）的列表的部分中随机选择元素。
     *
     * 该方法运行在线性时间。 如果指定的列表没有实现RandomAccess接口并且很大，则此实现将混合之前将指定的列表转储到数组中，
     * 并将该混洗的数组转储回列表中。 这避免了将“顺序访问”列表进行混洗所产生的二次行为。
     *
     ...
     * 从从第一个元素运行到当前位置（包括）的列表的部分中随机选择元素。
     *
     * 该方法运行在线性时间。 如果指定的列表没有实现RandomAccess接口，并且很大，
     * 那么这个实现会将指定的列表转储到数组中，然后再进行混洗，然后将该混洗后的数组转储回列表中。
     * 这避免了将“顺序访问”列表进行混洗所产生的二次行为。
     *
    ...
    /**
     * 根据其元素的自然顺序返回给定集合的最小元素。 集合中的所有元素必须实现Comparable界面。
     * 此外，集合中的所有元素必须相互可比较 （即， e1.compareTo(e2)不得为集合中的任何元素e1和e2提供ClassCastException）。
    ...
    /**
     * 根据指定的比较器引发的顺序返回给定集合的最小元素。 集合中的所有元素必须由指定的比较器相互比较
     * （即， comp.compare(e1, e2)不得为集合中的任何元素e1和e2引发ClassCastException）。
     * 该方法遍历整个集合，因此它需要与集合的大小成比例的时间。
     *
     * Returns the minimum element of the given collection, according to the
     ...
     * 此外，集合中的所有元素必须相互可比较 （即e1.compareTo(e2)不得为集合中的任何元素e1和e2投掷ClassCastException）。
     * 该方法遍历整个集合，因此它需要与集合的大小成比例的时间。
     ...
     * （即， comp.compare(e1, e2)不得为集合中的任何元素e1和e2投放ClassCastException）。
     * 该方法遍历整个集合，因此它需要与集合的大小成比例的时间。
     * Returns the maximum element of the given collection, according to the
     * order induced by the specified comparator.  All elements in the
     * collection must be <i>mutually comparable</i> by the specified
    ...
    /**
     * 将指定列表中的元素旋转指定的距离。 调用此方法后，在索引i的元件将是元件预先在索引(i - distance) list.size() MOD，
     * 用于0和list.size()-1（含）之间的i所有值。 （此方法对列表的大小没有影响。）
     * 例如，假设list包括[t, a, n, k, s] 。 在调用Collections.rotate(list, 1) （或Collections.rotate(list, -4) ）
     * 后， list将包括[s, t, a, n, k] 。
     *
     * 请注意，该方法可以有效地应用于子列表以移动列表中的一个或多个元素，同时保留剩余元素的顺序。
     * 例如，以下成语将索引j处的元素向前移动到位置k （必须大于或等于j ）：
     *
     *   Collections.rotate(list.subList(j, k+1), -1); 为了使这个具体，假设list包括[a, b, c, d, e] 。
     *   要将索引1 （ b ）上的元素向前移动两个位置，请执行以下调用：
     *   Collections.rotate(l.subList(1, 4), -1); 得到的列表是[a, c, d, b, e] 。
     * 要向前移动多个元素，请增加旋转距离的绝对值。 要向后移动元素，请使用正移位距离。
     *
     * 如果指定的列表较小或实现了RandomAccess接口，则该实现将第一个元素交换到应该进行的位置，
     * 然后重复地将移位的元素交换到应该进行的位置，直到被替换的元素被交换到第一个元素中。
     * 如果需要，在第二和连续元件上重复该过程，直到旋转完成。 如果指定的列表很大并且没有实现RandomAccess接口，
     * 则该实现将列表分成索引-distance mod size周围的两个子列表视图。 然后在每个子列表视图中调用reverse(List)方法，
     * 最后在整个列表中调用它。 有关这两种算法的更完整的描述，
     * 请参见Jon Bentley 编程珍珠的第2.3节（Addison-Wesley，1986）。
     *
     ...
     * 更正式地，返回最高指数i ，使source.subList(i, i+target.size()).equals(target) ，或-1如果没有这样的索引。
     * （如果target.size() > source.size()返回-1）
    ...
    /**
     * 返回指定集合的不可修改视图。 该方法允许模块向用户提供对内部集合的“只读”访问。
     * 对返回的集合“读取”查询操作到指定的集合，并尝试修改返回的集合，无论是直接还是通过其迭代器，
     * 导致UnsupportedOperationException 。
     * 返回的集合不会通过hashCode并将操作等同于后台集合，而是依赖于Object的equals和hashCode方法。
     * 在后台集合是集合或列表的情况下，必须保留这些操作的合同。
     *
...
/**
    ...
    /**
     *
     * 返回由指定地图支持的集合。 结果集显示与背景映射相同的排序，并发和性能特征。
     * 在本质上，此工厂方法提供了一种Set对应于任何执行Map实现。 已经有Set实现（例如HashMap或TreeMap ）
     * 的Map实现没有必要使用此方法。
     * 在该方法返回的集合上的每个方法调用都会在后台映射或其keySet视图上完成一个方法调用，但有一个例外。
     * addAll方法在辅助地图上以put调用的顺序实现。
     *
     * 在调用此方法时，指定的映射必须为空，并且在此方法返回后不能直接访问。
     * 如果映射创建为空，直接传递给此方法，并且不保留对映射的引用，则保证这些条件，如以下代码片段所示：
     *
    ...
    /**
     * 返回Deque作为先进先出（ Lifo ） Queue的视图 。 方法add被映射到push，remove映射到pop等。
     * 当您想使用需要Queue但需要Lifo订购的方法时，此视图可能很有用。
     * 通过此方法返回的队列上的每个方法调用都将在支持deque上完成一个方法调用，但有一个例外。
     * addAll方法被实现为一个addFirst对支持deque的调用的序列。
     *
