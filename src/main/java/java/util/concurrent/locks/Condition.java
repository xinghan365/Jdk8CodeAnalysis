 ...
 * Condition实现可以提供Object监视器方法的行为和语义，例如有保证的通知顺序，
 * 或者在执行通知时不需要锁定。 如果一个实现提供了这样的专门的语义，那么实现必须记录这些语义。
 *
 * 需要注意的是Condition实例只是普通的对象，其本身作为一个目标synchronized语句，
 * 可以有自己的监视器wait和notification个方法调用。
 * 获取Condition实例的监视器锁或使用其监视方法与获取与该Condition相关联的Condition
 * 或使用其waiting和signalling方法没有特定关系。
 ...
 * 这对大多数应用程序几乎没有实际的影响，因为Condition应该始终在循环中等待，
 * 测试正在等待的状态谓词。 一个实现可以免除虚假唤醒的可能性，
 * 但建议应用程序员总是假定它们可以发生，因此总是等待循环。
 *
 * 条件等待（可中断，不可中断和定时）的三种形式在一些平台上的易用性和性能特征可能不同。
 * 特别地，可能难以提供这些特征并保持特定的语义，例如排序保证。
 * 此外，中断线程实际挂起的能力可能并不总是在所有平台上实现。
 *
 * 因此，不需要一个实现来为所有三种形式的等待定义完全相同的保证或语义，
 * 也不需要支持中断线程的实际暂停。
 *
 * 需要一个实现来清楚地记录每个等待方法提供的语义和保证，
 * 并且当一个实现确实支持线程挂起中断时，它必须遵守该接口中定义的中断语义。
 *
 * 由于中断通常意味着取消，并且检查中断通常是不频繁的，
 * 所以实现可以有利于通过正常方法返回来响应中断。
