...
/**
 * 此类实现Map接口利用哈希表，比较键（和值）时使用引用相等代替对象相等的。 换句话说，在IdentityHashMap中 ，当且仅当(k1==k2)时，两个键k1和k2被认为是相等的。 （正常Map实现（如HashMap ）当两个键k1和k2被认为是相等的当且仅当(k1==null ? k2==null : k1.equals(k2))。 ）
 ...
 *
 * 假设系统标识散列函数（ System.identityHashCode(Object) ）正确分散在这些存储区中，这个类提供了基本操作（ get和put ）的恒定时间性能。
 *
 ...
 *
 *   Map m = Collections.synchronizedMap(new IdentityHashMap(...)); 由所有这个类的“集合视图方法”返回的集合的iterator方法返回的迭代器是故障快速的 ：如果映射在迭代器创建之后的任何时间被结构化地修改，除了通过迭代器自己的remove方法，迭代器会抛出一个ConcurrentModificationException 。 因此，面对并发修改，迭代器将快速而干净地失败，而不是在未来未确定的时间冒着任意的非确定性行为。
 *
 ...
 *
 * 实现说明：这是一个简单的线性探测哈希表，如Sedgewick和Knuth的文本中所述。 阵列交替保持键和值。 （与使用单独的数组相比，这对于大型表具有更好的局部性。）对于许多JRE实现和操作组合，此类将产生比HashMap （使用链接而不是线性检测）更好的性能。
 *
